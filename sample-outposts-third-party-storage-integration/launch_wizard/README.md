...
- **Launch Template Creation**: Automatic launch template generation for recovery scenarios
- **Automated Recovery Setup**: Integration with autorestart tool for Outpost Server recovery
...
### Multiple iSCSI Targets Support

The Launch Wizard supports configuring multiple iSCSI targets for primary/backup functionality. When multiple targets are specified, they are formatted as a single `sanboot` command with automatic failover:

```bash
# Multiple targets will generate:
# sanboot iscsi:10.1.1.1::::iqn.2010-04.org.ipxe:primary \
#           iscsi:10.1.1.2::::iqn.2010-04.org.ipxe:backup
```

This allows iPXE to automatically try the first target and fall back to the second if the first fails, providing redundancy for SAN boot scenarios.
...
# Example with multiple iSCSI targets for primary/backup configuration
python -m launch_wizard generic iscsi --target-iqn iqn.2010-04.org.ipxe:primary --target-endpoint 10.1.1.1 --target-iqn iqn.2010-04.org.ipxe:backup --target-endpoint 10.1.1.2
```
...
### Guest OS Script Integration

Guest OS scripts can be used in conjunction with each vendor subcommand. You can specify one or more script files:
...
All vendor subcommands support the following options:

- `--guest-os-script`: Path to additional guest OS script files to execute
- `--target-iqn`: iSCSI Target Qualified Name (can be specified multiple times for multiple targets)
- `--target-endpoint`: iSCSI target endpoint in IP or IP:PORT format (can be specified multiple times)
...
6. **Instance Health Monitoring**: Wait for instance to reach running state
7. **Launch Template Integration**: Prompt and execute template_generator tool for template creation
8. **Status Reporting**: Report instance launch status and recovery setup completion
